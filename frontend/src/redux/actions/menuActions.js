...
export const getMenus = (id) => async (dispatch) => {
  try {
    ...
    if (response.data.data?.length > 0) {
      ...
      menuData = response.data.data.flatMap((doc) => doc.menu);
    }
...
// CREATE MENU
export const createMenu = ({ restaurantId, category }) => async (dispatch) => {
  try {
...
// ADD ITEM
export const addItemToMenu = ({ menuId, category, foodItemId, restaurantId }) => async (dispatch) => {
  try {
